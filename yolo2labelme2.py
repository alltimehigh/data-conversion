...
def yolo2labelme_single(txt_path, img_path, class_labels, out_dir, suffix):
    ...
    json_path = f"{base_name}_{suffix}.json"
    json_path = os.path.join(out_dir, json_path)

    with open(json_path, 'w') as f:
        f.write(json.dumps(result))
    
    img_out_path = os.path.join(out_dir, f"{base_name}_{suffix}{ext}")
...
def yolo2labelme(data, out=None, skip=False):
    ...
    for dir_type in ['train', 'val']:
        suffix = dir_type  # Use suffix to distinguish between train and val
        ...
        for filename in os.listdir(dir_path):
            ...
            if is_image_file(img_file):
                ...
                if os.path.exists(txt_file):
                    yolo2labelme_single(txt_file, img_file, class_labels, out, suffix)
                else:
                    if skip == False:
